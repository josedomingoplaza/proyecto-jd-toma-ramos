...
def GetCourseName(NRC):
    for a in data:
        if a[2] == str(NRC):
            return a[8]

def GetCourseCode(NRC):
    for a in data:
        if a[2] == (str(NRC)):
            return a[6]
...
course_amount = int(input("Input the amount of courses you would like to take: "))

combos = list(itertools.combinations(all_courses, course_amount))
...
for combo in comboslist:
    ...
    for a in pairs:
        check = CheckCompatibility(a[0], a[1])
        if check == False:
            removedcombos.append(combo)
            break
    if check == True:
        finalcombos.append(combo)

print("Valid Combos:")
for a in finalcombos:
    print(a)

print("")

print("Removed Combos:")
for a in removedcombos: print(a)
